export default function OrderHistory({ orders, products }) {
    ...
    const handleDetailsClick = (id, e) => {
        if (document.getElementById(id).classList.toggle("hidden")) e.target.innerText = "Detalle"
        else e.target.innerText = "Ocultar detalle"
    }
                    ...
                    <div key={order.id} className="mb-1">
                        <div className="flex px-3 py-2 border rounded-sm">
                            ...
                            <div className="w-2/5 pt-8 flex flex-col justify-end">
                                ...
                                <button onClick={(e) => handleDetailsClick(order.id, e)} className="w-full py-1 bg-gray-600 rounded-sm text-xs lg:text-base text-white">Detalle</button>
                            </div>
                        </div>


                        <div className="px-4 py-3 border border-t-0 bg-gray-200 hidden animate-grow origin-top" id={order.id}>
